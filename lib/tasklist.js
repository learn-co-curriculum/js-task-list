...
function displayList() {
	...
	$('#task_list').html(''); // blank out the list

	$.each(taskList, function(i, value){ //each iterates over all elements in the array one by one.
		$('#task_list').append('<li>'+(i + 1)+'. <span class="name">'+value+'</span></li>'); //add back each name in the list.
...
function addTask() {
	taskName = $('#task_name').val(); //store the currently typed task name value
...
function removeTask() {
	taskNum = $('#task_num').val(); //store the currently typed task number value
